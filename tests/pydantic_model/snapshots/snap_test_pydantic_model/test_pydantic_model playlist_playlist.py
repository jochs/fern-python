...
class Playlist(PlaylistCreateRequest):
    ...
    def _validate_playlist_id(cls, playlist_id: PlaylistId, values: Playlist.Partial) -> PlaylistId:
        for validator in Playlist.Validators._playlist_id_validators:
            playlist_id = validator(playlist_id, values=values)
        return playlist_id

    @pydantic.validator("owner_id")
    def _validate_owner_id(cls, owner_id: UserId, values: Playlist.Partial) -> UserId:
        for validator in Playlist.Validators._owner_id_validators:
            owner_id = validator(owner_id, values=values)
        return owner_id
